...
static inline void XLPS (const union uint512_u* x,const union uint512_u* y,union uint512_u* data)
{
    ...
    for (i=0; i<=7; i++)
    {
        data->QWORD[i]  = Ax[0][(r0 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[1][(r1 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[2][(r2 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[3][(r3 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[4][(r4 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[5][(r5 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[6][(r6 >> (i << 3)) & 0xFF];
        data->QWORD[i] ^= Ax[7][(r7 >> (i << 3)) & 0xFF];
    }
}
...
static inline void ROUND (int i, union uint512_u* Ki, union uint512_u* data)
{
    XLPS(Ki, (&C[i]), Ki);
    XLPS(Ki, data, data);
}

static inline void pad(u8* buf, size_t bufsize)
{
    u8 nbuf[STRIBOG_BLOCK_SIZE];

    if (bufsize >= STRIBOG_BLOCK_SIZE)
        return;

    memset(&nbuf, 0x00, sizeof nbuf);
    memcpy(&nbuf, buf, bufsize);
...
static void
g(union uint512_u *h, const union uint512_u *N, const u8 *m)
{
...
static inline void
stage2(union uint512_u h, union uint512_u N, union uint512_u Sigma,
			      const unsigned char *data)
{
    g(&h, &N, data);

    add512(&N, &buffer512, &N);
    add512(&Sigma, (const union uint512_u *) data, &Sigma);
}

static inline void
stage3(struct stribog_state *CTX)
{
    union uint512_u buf;

    memset(&buf, 0x00, sizeof buf);
    memcpy(&buf, &(CTX->buf), CTX->bufsize);
    memcpy(&(CTX->buf), &buf, sizeof(union uint512_u));

    memset(&buf, 0x00, sizeof buf);

    buf.QWORD[0] = CTX->bufsize << 3;

    pad((u8*)CTX,CTX->bufsize);

    g(&(CTX->h), &(CTX->N), (const unsigned char *) &(CTX->buf));

    add512(&(CTX->N), &buf, &(CTX->N));
    add512(&(CTX->Sigma), (const union uint512_u *) &CTX->buf[0],
           &(CTX->Sigma));
...
static int
stribog512_init(struct shash_desc *desc)
{
    struct stribog_state *CTX = shash_desc_ctx(desc);
    unsigned int i;

    memset(CTX, 0x00, sizeof(struct stribog_state));
    for (i = 0; i < 8; i++)
    {
...
static int
stribog256_init(struct shash_desc *desc)
{
    struct stribog_state *CTX = shash_desc_ctx(desc);
    unsigned int i;
...
int crypto_stribog_update(struct shash_desc *desc, const u8 *data, unsigned int len)
{
    struct stribog_state *CTX = shash_desc_ctx(desc);
    size_t chunksize;
    ...
    {
        stage2(CTX->h, CTX->N, CTX->Sigma, data);
    ...
    while (len)
    {
        ...
        memcpy(&CTX->buf[CTX->bufsize], data, chunksize);
        ...
        if (CTX->bufsize == 64)
        {
            stage2(CTX->h, CTX->N, CTX->Sigma, CTX->buf);
...
EXPORT_SYMBOL(crypto_stribog_update);


static int
stribog512_final(struct shash_desc *desc, u8 *hash)
{
    struct stribog_state *CTX = shash_desc_ctx(desc);
    stage3(CTX);
    ...
    memcpy(hash, &(CTX->hash.QWORD[0]), 64);
    memset(CTX, 0x00, sizeof (struct stribog_state));
    return 0;
}

static int
stribog256_final(struct shash_desc *desc, u8 *hash)
{
    struct stribog_state *CTX = shash_desc_ctx(desc);
    stage3(CTX);
    ...
    memcpy(hash, &(CTX->hash.QWORD[4]), 32);
    memset(CTX, 0x00, sizeof (struct stribog_state));
    return 0;
}

static struct shash_alg stribog_algs[2] = { {
	.digestsize	=	STRIBOG256_DIGEST_SIZE,
	.init		=	stribog256_init,
	.update		=	crypto_stribog_update,
	.final		=	stribog256_final,
	.descsize	=	sizeof(struct stribog_state),
	.base		=	{
		.cra_name	=	"stribog256",
	...
	.digestsize	=	STRIBOG512_DIGEST_SIZE,
	.init		=	stribog512_init,
	.update		=	crypto_stribog_update,
	.final		=	stribog512_final,
	.descsize	=	sizeof(struct stribog_state),
	.base		=	{
		.cra_name	=	"stribog512",
...
static int __init stribog_generic_mod_init(void)
{
	return crypto_register_shashes(stribog_algs, ARRAY_SIZE(stribog_algs));
}

static void __exit stribog_generic_mod_fini(void)
{
	crypto_unregister_shashes(stribog_algs, ARRAY_SIZE(stribog_algs));
}

module_init(stribog_generic_mod_init);
module_exit(stribog_generic_mod_fini);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Russian Stribog Secure Hash Algorithms");

MODULE_ALIAS("stribog");
MODULE_ALIAS("gost34112012");
